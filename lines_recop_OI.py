...
if meth == 'S':
    ...
    aab = lmfit.Parameter('sig_20', value=sig20,min=sig_inst)# 6.9208410271,vary=False) #5.313169359,vary=False) #,max=12.83402) 6.,vary=False)  sig20,min=sig_inst)  3.6617798,vary=False)
    ...
    x_one = l[np.where(l_init>l3)[0][0]-20:np.where(l_init<l2)[0][-1]+20]
    y_one = data_cor[np.where(l_init>l3)[0][0]-20:np.where(l_init<l2)[0][-1]+20]
    ...
    # Zone of the lines for doing the fit
    x_one = l[np.where(l_init>l11)[0][0]:np.where(l_init<l14)[0][-1]+20]
    y_one = data_cor[np.where(l_init>l11)[0][0]:np.where(l_init<l14)[0][-1]+20]
...
ep_1,ep_2,ep2_1,ep2_2 = refer_plot(path,data_head,l,l_init,data_cor,meth,linresu,oneresu,tworesu,l1,l2,l3,l4,l11,l12,l13,l14,std0,std1,z,erz)
...
if trigger == 'Y':
	...
	if os.path.exists(path+'v_sig_adj'+str(meth)+'_1.txt'): 
            os.remove(path+'v_sig_adj'+str(meth)+'_1.txt')
	np.savetxt(path+'v_sig_adj'+str(meth)+'_1.txt',np.c_[vS2,evS2,sigS2,esigS2],
	...
	plt.plot(l,gaus5,'b-')
	plt.plot(l,gaus6,'b-')
	plt.plot(l,gaus7,'b-',label='Narrow component')
	...
	plt.savefig(path+'adj_met'+str(meth)+'_full_1comp.pdf',formar='pdf',bbox_inches='tight',pad_inches=0.2)
	props = dict(boxstyle='round', facecolor='white', alpha=0.5)
	frame1.text(6350.,max(data_cor), textstr, fontsize=12,verticalalignment='top', bbox=props)
	plt.savefig(path+'adj_met'+str(meth)+'_full_1comp.png',bbox_inches='tight',pad_inches=0.2)
	...
	if trigger2 == 'N': 
	    print('The final plots are already printed and have been already saved!')
            np.savetxt(path+'fitone_best_values.txt',np.c_[resu1.data,resu1.best_fit,lin_data_fin,gaus3,gaus4,gaus5],fmt=('%5.10f','%5.10f','%5.10f','%5.10f','%5.10f','%5.10f'),header=('Real_data\tBest_fit\tLineal_fit\tNarrow_NII2\tNarrow_Halpha\tNarrow_NII1'))
	elif trigger2 == 'Y':
	    # Now we define the initial guesses and the constraints
	    ...
	    # PLOT AND PRINT THE RESULTS 
	    refer2 = broad_plot(path,data_head,l,l_init,data_cor,meth,trigger,linresu,oneresu,fin_fit,broadresu,l1,l2,l3,l4,l5,l6,l7,l8,l9,l10,l11,l12,l13,l14,std0,std1,z,erz)
	...
	if os.path.exists(path+'eps_adj'+str(meth)+'_2.txt'): os.remove(path+'eps_adj'+str(meth)+'_2.txt')
        ...
        try:
            # We determine the maximum flux of the fit for all the lines, and the velocity and sigma components
            max2S1 = fin2_fit[np.where(abs(twocompresu.values['mu_0']-l)<0.28)[0][0]] #max(fin2_fit[np.where(l_init>l3)[0][0]:np.where(l_init<l4)[0][-1]])
            max2S2 = fin2_fit[np.where(abs(twocompresu.values['mu_1']-l)<0.28)[0][0]] #max(fin2_fit[np.where(l_init>l1)[0][0]:np.where(l_init<l2)[0][-1]])
            max2N1 = fin2_fit[np.where(abs(twocompresu.values['mu_2']-l)<0.28)[0][0]] #max(fin2_fit[np.where(l_init>l9)[0][0]:np.where(l_init<l10)[0][-1]])
            max2Ha = fin2_fit[np.where(abs(twocompresu.values['mu_3']-l)<0.28)[0][0]] #max(fin2_fit[np.where(l_init>l7)[0][0]:np.where(l_init<l8)[0][-1]])
            max2N2 = fin2_fit[np.where(abs(twocompresu.values['mu_4']-l)<0.28)[0][0]] #max(fin2_fit[np.where(l_init>l5)[0][0]:np.where(l_init<l6)[0][-1]])
            max2O2 = fin2_fit[np.where(abs(twocompresu.values['mu_6']-l)<0.28)[0][0]] #max(fin2_fit[np.where(l_init>l13)[0][0]:np.where(l_init<l14)[0][-1]])
            max2O1 = fin2_fit[np.where(abs(twocompresu.values['mu_5']-l)<0.28)[0][0]] #max(fin2_fit[np.where(l_init>l11)[0][0]:np.where(l_init<l12)[0][-1]])
	except IndexError: 
	...
	# Save the velocity and sigma for all components
	if os.path.exists(path+'v_sig_adj'+str(meth)+'2.txt'): os.remove(path+'v_sig_adj'+str(meth)+'2.txt')
	np.savetxt(path+'v_sig_adj'+str(meth)+'2.txt',np.c_[v2S2,ev2S2,v20S2,ev20S2,sig2S2,esig2S2,sig20S2,esig20S2],
    		  ('%8.5f','%8.5f','%8.5f','%8.5f','%8.5f','%8.5f','%8.5f','%8.5f'),header=('v_ref2\tev_ref2\tv_2ref2\tev_2ref2\tsig_ref2\tesig_ref2\tsig_2ref2\tesig_2ref2'))
	...
	plt.plot(l,tgaus5,'b-')
	plt.plot(l,tgaus6,'b-')
	plt.plot(l,tgaus7,'b-',label='Narrow component')
	plt.plot(l,tgaus8,c='darkviolet',linestyle='-')
	plt.plot(l,tgaus9,c='darkviolet',linestyle='-')
	plt.plot(l,tgaus10,c='darkviolet',linestyle='-')
	plt.plot(l,tgaus11,c='darkviolet',linestyle='-')
	plt.plot(l,tgaus12,c='darkviolet',linestyle='-')
	plt.plot(l,tgaus13,c='darkviolet',linestyle='-')
	plt.plot(l,tgaus14,c='darkviolet',linestyle='-',label='Secondary component')
	...
	plt.savefig(path+'adj_met'+str(meth)+'_full_2comp.pdf',format='pdf',bbox_inches='tight',pad_inches=0.2)
	props = dict(boxstyle='round', facecolor='white', alpha=0.5)
	frame1.text(6350.,max(data_cor), textstr, fontsize=12,verticalalignment='top', bbox=props)
	plt.savefig(path+'adj_met'+str(meth)+'_full_2comp.png',bbox_inches='tight',pad_inches=0.2)

	########################################################################################################################
	trigger2 = input('Do the fit needs a broad Halpha component? ("Y"/"N"): ')
	if trigger2 == 'N': 
	    print('The final plots are already printed and have been already saved!')
            np.savetxt(path+'fittwo_best_values.txt',np.c_[twocompresu.data,twocompresu.best_fit,lin_data_fin,tgaus3,tgaus10,tgaus4,tgaus11,tgaus5,tgaus12],fmt=('%5.10f','%5.10f','%5.10f','%5.10f','%5.10f','%5.10f','%5.10f','%5.10f','%5.10f'),header=('Real_data\tBest_fit\tLineal_fit\tNarrow_NII2\tSecond_NII2\tNarrow_Halpha\tSecond_Halpha\tNarrow_NII1\tSecond_NII1'))
	elif trigger2 == 'Y':
            # Now we define the initial guesses and the constraints
	    newxb = l[np.where(l_init>l9)[0][0]:np.where(l_init<l6)[0][-1]]
	    newyb = data_cor[np.where(l_init>l9)[0][0]:np.where(l_init<l6)[0][-1]]
	    ...
	    # PLOT AND PRINT THE RESULTS 
	    refer2 = broad_plot(path,data_head,l,l_init,data_cor,meth,trigger,linresu,tworesu,fin2_fit,twobroadresu,l1,l2,l3,l4,l5,l6,l7,l8,l9,l10,l11,l12,l13,l14,std0,std1,z,erz)
