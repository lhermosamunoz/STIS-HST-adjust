...
#
path  = "/mnt/data/lhermosa/L2_palomar_stsub/"
#FILE  = input('Name of the spectra? (ex. "NGC_FINAL.dat"): ')
#dataFile = np.genfromtxt(path+FILE)       #'/mnt/data/lhermosa/HLA_data/NGC3245/O57205030_STISCCD_G750M/ext_spec_combin_crop.fits')       # Open the fit file to read the information

# Select all the files (sustitute of the previous lines)
#gal = ['NGC2685','NGC3245','NGC4374','NGC4486','NGC4552','NGC4594','NGC4698','NGC4736']
gal = ['NGC2685','NGC4698','NGC4736']
allPaths = []
for i in gal:
   allPaths.append(path+i+'_FINAL.dat')
   print('Adding '+path+i+'_FINAL.dat to parentFolds...')
   if not os.path.exists(path+i+'_results'):
        os.mkdir(path+i+'_results')
        print('Folder '+path+i+'_results created in path')
...
plate_scale = 0.58 # arcsec per pix
ang_to_pix = 2.5   # Angstrom per pixel - spectral resolution
fwhm = 2*np.sqrt(2*np.log(2)) # por sigma
sigma_per_arc = 82  # km/s per arcsec derived from the FWHM per arcsec
pix_to_v = 47.7 # km/s per arcsec BASADO EN QUE LA SIGINST ES 42 km/s

sig_inst = 2.2	# A  sacado de Ho et al 2009 (datos de 1995 y 1997)
#sig_inst = 2.5
sig_inicial = 3.

# Fitting process for all galaxies in a row
index_gal = 0
for galaxy in allPaths:
    # Define the path to save the data
    dataFile = np.genfromtxt(galaxy)
    parentFold = path+gal[index_gal]+'_results/'
    
    print('Start fitting the spectra of the galaxy '+gal[index_gal]+'... ')
    ...
    if not os.path.exists(path+'ranges_'+gal[index_gal]+'.txt'):
        ...
        t = np.genfromtxt(path+'ranges_'+gal[index_gal]+'.txt')
        l1 = t[0,]
        l2 = t[1,]
        l3 = t[2,]
        l4 = t[3,]
    ...
    if not os.path.exists(path+gal[index_gal]+'_results/'+meth+'_method'):
        os.mkdir(path+gal[index_gal]+'_results/'+meth+'_method')

    parentFold = path+gal[index_gal]+'_results/'+meth+'_method/'
    
    # -------------
    if meth == 'S':
        cd1 = lmfit.Parameter('mu_0', value=mu0)
        de = lmfit.Parameter('sig_0', value=sig0,min=sig_inst)# value=3.145085, vary=False) # 3.37156527,vary=False) #, value=3.3813299,vary=False)#,max=minbroad/2.)# value=sig0,min=sig_inst) 
        ...
        aab = lmfit.Parameter('sig_20', value=sig20,min=sig_inst,max=10.)# 6.9208410271,vary=False) #5.313169359,vary=False) #,max=12.83402) 6.,vary=False)  sig20,min=sig_inst)  3.6617798,vary=False)
        ...
        cd1 = lmfit.Parameter('mu_0', value=mu5)
        ...
        hi = lmfit.Parameter('amp_1', value=amp6,min=0.,expr='amp_0*(1./2.96)')
        # second components
        aaa = lmfit.Parameter('mu_20', value=mu5)
        aab = lmfit.Parameter('sig_20', value=sig25,min=sig_inst,max=10.)
        ...
        aaf = lmfit.Parameter('amp_21', value=amp26,min=0.,expr='amp_20*(1./2.96)')
        # Zone of the lines for doing the fit
    ...
    lmfit.model.save_modelresult(oneresu, parentFold+str(meth)+'I_modelresult.sav')
    lmfit.model.save_modelresult(tworesu, parentFold+str(meth)+'I_twocomps_modelresult.sav')
    with open(parentFold+'fit_'+str(meth)+'I_result.txt', 'w') as fh:
        fh.write(oneresu.fit_report())
    with open(parentFold+'fit_two'+str(meth)+'I_result.txt', 'w') as fh:
    ...
    if oneresu.chisqr < tworesu.chisqr: 
        ...
        ep_1,ep_2,ep2_1,ep2_2 = refer_plot(parentFold,l,data_cor,meth,linresu,oneresu,tworesu,l1,l2,l3,l4,l11,l12,l13,l14,std0,std1)
    ...
    if trigger == 'Y':
   	params = lmfit.Parameters()
	    ...
	    wy = lmfit.Parameter('amp_6', value=amp6,min=0.,expr='amp_5*(1./2.96)')
	    ...
	    params.add_many(sl,it,rs,st,tu,cd1,de,ef,fg,gh,hi,ij,jk,kl,lm,mn,no,op,pq,qr,uv,vw,wy)
	...
	np.savetxt(parentFold+'eps_adj'+str(meth)+'_1.txt',np.c_[stdf_s2/stadev,stdf_s1/stadev,stdf_n2/stadev,stdf_ha/stadev,stdf_n1/stadev,resu1.chisqr],
	...
	maxfS1 = fin_fit[np.where(abs(resu1.values['mu_0']-l)<0.5)[0][0]] #max(fin_fit[np.where(l>l3)[0][0]:np.where(l<l4)[0][-1]])
	maxfS2 = fin_fit[np.where(abs(resu1.values['mu_1']-l)<0.511)[0][0]] #max(fin_fit[np.where(l>l1)[0][0]:np.where(l<l2)[0][-1]])
	maxfN1 = fin_fit[np.where(abs(resu1.values['mu_2']-l)<0.5)[0][0]] #max(fin_fit[np.where(l>l9)[0][0]:np.where(l<l10)[0][-1]])
	maxfHa = fin_fit[np.where(abs(resu1.values['mu_3']-l)<0.5)[0][0]] #max(fin_fit[np.where(l>l7)[0][0]:np.where(l<l8)[0][-1]])
	maxfN2 = fin_fit[np.where(abs(resu1.values['mu_4']-l)<0.5)[0][0]] #max(fin_fit[np.where(l>l5)[0][0]:np.where(l<l6)[0][-1]])
	maxfO1 = fin_fit[np.where(abs(resu1.values['mu_5']-l)<0.5)[0][0]] #max(fin_fit[np.where(l>l11)[0][0]:np.where(l<l12)[0][-1]])
	maxfO2 = fin_fit[np.where(abs(resu1.values['mu_6']-l)<0.5)[0][0]] #max(fin_fit[np.where(l>l13)[0][0]:np.where(l<l14)[0][-1]])
	
	# one component
	'''
	try:
	    assert(oneresu.params['sig_0'].stderr != None)
	    esigS2 = pix_to_v*np.sqrt(oneresu.values['sig_0']*oneresu.params['sig_0'].stderr)/(np.sqrt(oneresu.values['sig_0']**2-sig_inst**2))
	except AssertionError:
	    esigS2 = 0.
	    print('Something happened and the error of sigma in the narrow component could not be estimated!')
	finally: 
	...
	if os.path.exists(parentFold+'v_sig_adj'+str(meth)+'_1.txt'): 
	    os.remove(parentFold+'v_sig_adj'+str(meth)+'_1.txt')
	np.savetxt(parentFold+'v_sig_adj'+str(meth)+'_1.txt',np.c_[vS2,evS2,sigS2,esigS2],
		   ('%8.5f','%8.5f','%8.5f','%8.5f'),header=('v_ref\tev_ref\tsig_ref\tesig_ref'))
	if os.path.exists(parentFold+gal[index_gal]+'_fluxes_'+str(meth)+'_1.txt'): os.remove(parentFold+gal[index_gal]+'_fluxes_'+str(meth)+'_1.txt')
	np.savetxt(parentFold+gal[index_gal]+'_fluxes_'+str(meth)+'_1.txt',np.c_[maxfS2/maxfHa,maxfS1/maxfHa,maxfN2/maxfHa,maxfHa,maxfN1/maxfHa,maxfO2/maxfHa,maxfO1/maxfHa],('%8.6f','%8.6f','%8.6f','%8.15f','%8.6f','%8.6f','%8.6f'),header=('SII_6731\tSII_6716\tNII_6584\tiHalpha\tNII_6548\tOI_6363\tOI_6300'))
	################################################ PLOT #####################################################
	plt.close('all')
	# MAIN plot
	...
	plt.plot(l,gaus1,c='g',linestyle='-')
	plt.plot(l,gaus2,c='g',linestyle='-')
	plt.plot(l,gaus3,c='g',linestyle='-')
	plt.plot(l,gaus4,c='g',linestyle='-')
	plt.plot(l,gaus5,c='g',linestyle='-')
	plt.plot(l,gaus6,c='g',linestyle='-')
	plt.plot(l,gaus7,c='g',linestyle='-')#,label='Narrow component')
	...
	plt.plot(l[std0:std1],data_cor[std0:std1],'darkorange')	# Zone where the stddev is calculated
	frame1.set_xticklabels([]) 			# Remove x-tic labels for the first frame
	plt.ylabel('Flux ($\mathrm{erg/s/cm^{2} / \AA}$)',fontsize=19)
	plt.tick_params(axis='both', labelsize=17)
	plt.xlim(l[0],l[-1])
        plt.gca().yaxis.set_major_locator(plt.MaxNLocator(prune='lower'))
        plt.legend(['Narrow'],handlelength=0,handletextpad=0,loc='upper right',fontsize=24)
        plt.text(6300,maxfN1,'PALOMAR',fontsize=24)
	...
	plt.savefig(parentFold+'adj_met'+str(meth)+'_full_1comp.pdf',formar='pdf',bbox_inches='tight',pad_inches=0.2)
	props = dict(boxstyle='round', facecolor='white', alpha=0.5)
	frame1.text(6350.,max(data_cor), textstr, fontsize=12,verticalalignment='top', bbox=props)
	plt.savefig(parentFold+'adj_met'+str(meth)+'_full_1comp.png',bbox_inches='tight',pad_inches=0.2)
	...
	if trigger2 == 'N': 
	    ...
	    np.savetxt(parentFold+'fitone_best_values.txt',np.c_[resu1.data,resu1.best_fit,lin_data_fin,gaus3,gaus4,gaus5],fmt=('%5.10f','%5.10f','%5.10f','%5.10f','%5.10f','%5.10f'),header=('Real_data\tBest_fit\tLineal_fit\tNarrow_NII2\tNarrow_Halpha\tNarrow_NII1'))
	elif trigger2 == 'Y':
	    # Now we define the initial guesses and the constraints
		...
		paramsbH.add_many(sl,it,ab,bc,yz,rs,st,tu,cd1,de,ef,fg,gh,hi,ij,jk,kl,lm,mn,no,op,pq,qr,uv,vw,wy)
	    ...
	    aau = lmfit.Parameter('amp_26', value=amp26,min=0.,expr='amp_25*(1./2.96)')
	    ...
	    # Now we define the initial guesses and the constraints
	    cd1 = lmfit.Parameter('mu_0', value=mu0,expr='mu_5*(6731./6300.)')
	    de = lmfit.Parameter('sig_0', value=sig0,expr = 'sig_5')
	    ef = lmfit.Parameter('amp_0',value=amp0,min=0.)
	    ...
	    params2c.add_many(sl,it,rs,st,tu,cd1,de,ef,fg,gh,hi,ij,jk,kl,lm,mn,no,op,pq,qr,uv,vw,wy,aap,aaq,aar,aas,aat,aau,aaa,aab,aac,aad,aae,aaf,aag,aah,aai,aaj,aak,aal,aam,aan,aao)

	# FIT
	twocompresu = twocomp_mod.fit(data_cor,params2c,x=l)
	
	lmfit.model.save_modelresult(twocompresu, parentFold+'two_modelresult.sav')
	with open(parentFold+'fit_two_result.txt', 'w') as fh:
	...
	if os.path.exists(parentFold+'eps_adj'+str(meth)+'_2.txt'): os.remove(parentFold+'eps_adj'+str(meth)+'_2.txt')
	np.savetxt(parentFold+'eps_adj'+str(meth)+'_2.txt',np.c_[stdf2_s2/stadev,stdf2_s1/stadev,stdf2_n2/stadev,stdf2_ha/stadev,stdf2_n1/stadev,twocompresu.chisqr],
		   ('%8.5f','%8.5f','%8.5f','%8.5f','%8.5f','%8.5f'),header=('SII2\tSII1\tNII2\tHa\tNII1\tChi2'))

	try:
	    # We determine the maximum flux of the fit for all the lines, and the velocity and sigma components
	    max2S1 = fin2_fit[np.where(abs(twocompresu.values['mu_0']-l)<0.5)[0][0]] #max(fin2_fit[np.where(l>l3)[0][0]:np.where(l<l4)[0][-1]])
	    max2S2 = fin2_fit[np.where(abs(twocompresu.values['mu_1']-l)<0.511)[0][0]] #max(fin2_fit[np.where(l>l1)[0][0]:np.where(l<l2)[0][-1]])
	    max2N1 = fin2_fit[np.where(abs(twocompresu.values['mu_2']-l)<0.5)[0][0]] #max(fin2_fit[np.where(l>l9)[0][0]:np.where(l<l10)[0][-1]])
	    max2Ha = fin2_fit[np.where(abs(twocompresu.values['mu_3']-l)<0.5)[0][0]] #max(fin2_fit[np.where(l>l7)[0][0]:np.where(l<l8)[0][-1]])
	    max2N2 = fin2_fit[np.where(abs(twocompresu.values['mu_4']-l)<0.5)[0][0]] #max(fin2_fit[np.where(l>l5)[0][0]:np.where(l<l6)[0][-1]])
	    max2O2 = fin2_fit[np.where(abs(twocompresu.values['mu_6']-l)<0.5)[0][0]] #max(fin2_fit[np.where(l>l13)[0][0]:np.where(l<l14)[0][-1]])
	    max2O1 = fin2_fit[np.where(abs(twocompresu.values['mu_5']-l)<0.5)[0][0]] #max(fin2_fit[np.where(l>l11)[0][0]:np.where(l<l12)[0][-1]])
	except IndexError: 
	...
	if tworesu.params['sig_0'].stderr == None: 
	     ...
	     esig2S2 = pix_to_v*(twocompresu.values['sig_0']*tworesu.params['sig_0'].stderr)/(np.sqrt(twocompresu.values['sig_0']**2-sig_inst**2))
	...
	if tworesu.params['sig_20'].stderr == None:
	    ...
	    esig20S2 = pix_to_v*(twocompresu.values['sig_20']*tworesu.params['sig_20'].stderr)/(np.sqrt(twocompresu.values['sig_20']**2-sig_inst**2))
	...
	# Save the velocity and sigma for all components
	if os.path.exists(parentFold+'v_sig_adj'+str(meth)+'_2.txt'): 
           os.remove(parentFold+'v_sig_adj'+str(meth)+'_2.txt')
	np.savetxt(parentFold+'v_sig_adj'+str(meth)+'_2.txt',
                   np.c_[v2S2,ev2S2,v20S2,ev20S2,sig2S2,esig2S2,sig20S2,esig20S2],
		   ('%8.5f','%8.5f','%8.5f','%8.5f','%8.5f','%8.5f','%8.5f','%8.5f'),
                   header=('v_ref2\tev_ref2\tv_2ref2\tev_2ref2\tsig_ref2\tesig_ref2\tsig_2ref2\tesig_2ref2'))
	if os.path.exists(parentFold+gal[index_gal]+'_fluxes_'+str(meth)+'_2.txt'): 
	   os.remove(parentFold+gal[index_gal]+'_fluxes_'+str(meth)+'_2.txt')
	np.savetxt(parentFold+gal[index_gal]+'_fluxes_'+str(meth)+'_2.txt',
                   np.c_[max2S2/max2Ha,max2S1/max2Ha,max2N2/max2Ha,max2Ha,max2N1/max2Ha,max2O2/max2Ha,max2O1/max2Ha],
                   ('%8.6f','%8.6f','%8.6f','%8.15f','%8.6f','%8.6f','%8.6f'),
                   header=('SII_6731\tSII_6716\tNII_6584\tHalpha\tNII_6548\tOI_6363\tOI_6300'))
	############################################### PLOT #####################################################
	plt.close('all')
	# MAIN plot
	fig1   = plt.figure(1,figsize=(10, 9))
	frame1 = fig1.add_axes((.1,.25,.85,.6)) 	     # xstart, ystart, xend, yend [units are fraction of the image frame, from bottom left corner]
	plt.plot(l,data_cor,'k')			     # Initial data
	plt.plot(l,(linresu.values['slope']*l+linresu.values['intc']),c='y',linestyle=(0, (5, 8)),label='Linear fit')
	plt.plot(l,tgaus1,c='g',linestyle='-')
	plt.plot(l,tgaus2,c='g',linestyle='-')
	plt.plot(l,tgaus3,c='g',linestyle='-')
	plt.plot(l,tgaus4,c='g',linestyle='-')
	plt.plot(l,tgaus5,c='g',linestyle='-')
	plt.plot(l,tgaus6,c='g',linestyle='-')
	plt.plot(l,tgaus7,c='g',linestyle='-')#,label='Narrow component')
	plt.plot(l,tgaus8,c='dodgerblue',linestyle='-')
	plt.plot(l,tgaus9,c='dodgerblue',linestyle='-')
	plt.plot(l,tgaus10,c='dodgerblue',linestyle='-')
	plt.plot(l,tgaus11,c='dodgerblue',linestyle='-')
	plt.plot(l,tgaus12,c='dodgerblue',linestyle='-')
	plt.plot(l,tgaus13,c='dodgerblue',linestyle='-')
	plt.plot(l,tgaus14,c='dodgerblue',linestyle='-')#,label='Secondary component')
	...
	plt.xlim(l[0],l[-1])
        plt.gca().yaxis.set_major_locator(plt.MaxNLocator(prune='lower'))
	plt.legend(['Narrow + Second'],handlelength=0,handletextpad=0,loc='best',fontsize=24)
        plt.text(6300,max2N1,'PALOMAR',fontsize=24)
	...
	plt.savefig(parentFold+'adj_met'+str(meth)+'_full_2comp.pdf',format='pdf',bbox_inches='tight',pad_inches=0.2)
	props = dict(boxstyle='round', facecolor='white', alpha=0.5)
	frame1.text(6350.,max(data_cor), textstr, fontsize=12,verticalalignment='top', bbox=props)
	plt.savefig(parentFold+'adj_met'+str(meth)+'_full_2comp.png',bbox_inches='tight',pad_inches=0.2)

	###########################################################################################################
	trigger2 = input('Do the fit needs a broad Halpha component? ("Y"/"N"): ')
	if trigger2 == 'N': 
	    print('The final plots are already printed and have been already saved!')
	    np.savetxt(parentFold+'fittwo_best_values.txt',np.c_[twocompresu.data,twocompresu.best_fit,lin_data_fin,tgaus3,tgaus10,tgaus4,tgaus11,tgaus5,tgaus12],fmt=('%5.10f','%5.10f','%5.10f','%5.10f','%5.10f','%5.10f','%5.10f','%5.10f','%5.10f'),header=('Real_data\tBest_fit\tLineal_fit\tNarrow_NII2\tSecond_NII2\tNarrow_Halpha\tSecond_Halpha\tNarrow_NII1\tSecond_NII1'))
	elif trigger2 == 'Y':
	    # Now we define the initial guesses and the constraints
	    newxb = l[np.where(l>l9)[0][0]:np.where(l<l6)[0][-1]]
	    newyb = data_cor[np.where(l>l9)[0][0]:np.where(l<l6)[0][-1]]
		...
		paramsbH.add_many(sl,it,rs,st,tu,ab,bc,yz,cd1,de,ef,fg,gh,hi,ij,jk,kl,lm,mn,no,op,pq,qr,uv,vw,wy,aap,aaq,aar,aas,aat,aau,aaa,aab,aac,aad,aae,aaf,aag,aah,aai,aaj,aak,aal,aam,aan,aao)
